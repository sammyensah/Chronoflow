...
/* ═══ EXTENSION ISLANDS : AMBIANCE AUDIO ═══ */
const Ambience = {
  tracks: {
    lofi: new Audio('https://stream.zeno.fm/0r0xa792kwzuv'), // Flux Lofi direct
  ...
  toggle(type) {
    ...
    if (this.active === type) {
      this.tracks[type].pause();
      this.active = null;
      btn.classList.remove('active');
    } else {
      // Coupe l'autre piste si elle joue
      if (this.active) {
        this.tracks[this.active].pause();
        document.getElementById(`btn-${this.active}`).classList.remove('active');
      }
      this.tracks[type].loop = true;
      this.tracks[type].play().catch(e => console.log("Lecture bloquée : cliquez sur la page d'abord."));
      this.active = type;
...
/* ═══ AMÉLIORATION XP VISUELLE ═══ */
// On surcharge légèrement ta fonction addXpV2 pour ajouter l'effet flottant
const originalAddXpV2 = addXpV2;
window.addXpV2 = function(n) {
  originalAddXpV2(n); // Exécute ton code original
  
  // Ajoute la petite bulle d'XP flottante
  const p = document.createElement('div');
  p.innerText = `✨ +${n} XP`;
  p.style.cssText = `
    position: fixed; bottom: 100px; right: 50px;
    color: #70d6ff; font-weight: 800; font-family: 'Nunito';
    animation: xpJump 1s forwards; pointer-events: none; z-index: 9999;
  `;
