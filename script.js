...
const LANGS = {
  fr:{
    ...
    max_study_day:'Max révision/jour (h)',max_leisure_day:'Max loisirs/jour (h)',break_between:'Pause entre événements (min)',add_course:'Ajouter un cours',
  },
  en:{
    ...
    max_study_day:'Max study/day (h)',max_leisure_day:'Max leisure/day (h)',break_between:'Break between events (min)',add_course:'Add a course',
  },
  de:{
...
function updateTemplates(){
  ...
  const TDAYS=[T('day_monday'),T('day_tuesday'),T('day_wednesday'),T('day_thursday'),T('day_friday'),T('day_saturday')];
  ...
  if(S.template==='student'){const courseSel=(sel,selVal)=>'<select class="sel">'+TDAYS.map((d,i)=>'<option value="'+(i+1)+'" '+(selVal===i+1?'selected':'')+'>'+d+'</option>').join('')+'</select>';const courseRows=(S.templateData.courses||[]).length===0?'<div class="course-row">'+courseSel('',1)+'<input type="time" class="tinp" value="08:00" step="300"><input type="time" class="tinp" value="10:00" step="300"><input type="text" class="sinp" placeholder="'+T('revise_subject')+'"><button class="btn-remove" onclick="this.parentElement.remove()">✕</button></div>':(S.templateData.courses||[]).map(course=>'<div class="course-row">'+courseSel('',course.day)+'<input type="time" class="tinp" value="'+(course.start||'08:00')+'" step="300"><input type="time" class="tinp" value="'+(course.end||'10:00')+'" step="300"><input type="text" class="sinp" value="'+(course.subject||'')+'" placeholder="'+T('revise_subject')+'"><button class="btn-remove" onclick="this.parentElement.remove()">✕</button></div>').join('');html+='<div class="template-form-card"><h3 style="margin-bottom:1rem">🎓 '+T('pcStudent')+'</h3><div class="tpl-limits"><div class="form-group"><label>'+T('max_study_day')+'</label><input type="number" id="tplMaxStudy" value="'+(S.templateData.maxStudy||4)+'" min="1" max="16"></div><div class="form-group"><label>'+T('max_leisure_day')+'</label><input type="number" id="tplMaxLeisure" value="'+(S.templateData.maxLeisure||3)+'" min="0" max="16"></div><div class="form-group"><label>'+T('break_between')+'</label><input type="number" id="tplBreak" value="'+(S.templateData.breakMin||10)+'" min="5" max="60" step="5"></div></div><div id="tplCC" style="margin-bottom:.75rem">'+courseRows+'</div><button class="btn-secondary" onclick="addTplCourse()" style="margin-bottom:.75rem">+ '+T('add_course')+'</button><button class="btn-primary" onclick="saveTplStudent()">'+T('save')+'</button></div>';}
...
function addTplCourse(){const c=document.getElementById('tplCC');if(!c)return;const DAYS=[T('day_monday'),T('day_tuesday'),T('day_wednesday'),T('day_thursday'),T('day_friday'),T('day_saturday')];const row=document.createElement('div');row.className='course-row';row.innerHTML='<select class="sel">'+DAYS.map((d,i)=>'<option value="'+(i+1)+'">'+d+'</option>').join('')+'</select><input type="time" class="tinp" value="08:00" step="300"><input type="time" class="tinp" value="10:00" step="300"><input type="text" class="sinp" placeholder="'+T('revise_subject')+'"><button class="btn-remove" onclick="this.parentElement.remove()">✕</button>';c.appendChild(row);}
